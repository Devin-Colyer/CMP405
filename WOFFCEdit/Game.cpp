...
void Game::Initialize(HWND window, int width, int height, RECT viewportDimensions)
{
	m_window = window;
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	if (m_camera) {
		// camera input
		m_camera->UpdateViewportRect(m_screenDimensions);
		m_camera->HandleInput(deltaTime, m_InputCommands);
	...
	if (m_InputCommands.lmbDown) {
		if (GetFocus() == GetParent(m_window)) {
			bool mouseInWindowX = (m_InputCommands.mouseX > m_screenDimensions.left && m_InputCommands.mouseX < m_screenDimensions.right),
				mouseInWindowY = (m_InputCommands.mouseY > m_screenDimensions.top && m_InputCommands.mouseY < m_screenDimensions.bottom);

			if (mouseInWindowX && mouseInWindowY) {
				m_selectedObject = MouseSelect();
				m_InputCommands.lmbDown = false;
			}
		}
	}
...
void Game::DrawHUD()
{
	...
	Vector3 camPos, camOri;
	if (m_camera) {
		camPos = m_camera->GetCamPosition();
		camOri = m_camera->GetCamOrientation();
	}

	std::wstring var = L"Cam X: " + std::to_wstring(camPos.x) + L", Cam Y: " + std::to_wstring(camPos.y) + L", Cam Z: " + std::to_wstring(camPos.z);
	std::wstring var2 = L"Cam Ptich: " + std::to_wstring(camOri.x) + L", Cam Yaw: " + std::to_wstring(camOri.y) + L", Cam Roll: " + std::to_wstring(camOri.z);
	std::wstring var3 = L"FPS: " + std::to_wstring(m_timer.GetFramesPerSecond());
	
	m_font->DrawString(m_sprites.get(), var.c_str(), XMFLOAT2(50, 10), Colors::Yellow, 0.f, XMFLOAT2(0.f, 0.f), 0.75f);
	m_font->DrawString(m_sprites.get(), var2.c_str(), XMFLOAT2(50, 25), Colors::Yellow, 0.f, XMFLOAT2(0.f, 0.f), 0.75f);
	m_font->DrawString(m_sprites.get(), var3.c_str(), XMFLOAT2(50, 40), Colors::Yellow, 0.f, XMFLOAT2(0.f, 0.f), 0.75f);
	m_sprites->End();
}
...
void Game::SetCameraSettings(float MoveSpeed, float RotationSpeed)
{
	m_camera->SetMoveSpeed(MoveSpeed);
	m_camera->SetRotationRate(RotationSpeed);
}

float Game::GetCameraMoveSpeed()
{
	return m_camera->GetMoveSpeed();
}

float Game::GetCameraRotationSpeed()
{
	return m_camera->GetRotationRate();
}

void Game::SetScreenDimensions(RECT in)
{
	m_screenDimensions = in;
}

int Game::MouseSelect()
{
	RECT sceenDims;
	GetClientRect(m_window, &sceenDims);

	int selectedID = -1;
	float selectedDist = INFINITY, shortestDist = INFINITY;

	// setup near and far planes of frustrum w/ mouse
	const XMVECTOR nearSource = XMVectorSet(m_InputCommands.inWindowMouseX, m_InputCommands.inWindowMouseY, 0.f, 1.f);
	const XMVECTOR farSource = XMVectorSet(m_InputCommands.inWindowMouseX, m_InputCommands.inWindowMouseY, 1.f, 1.f);

	for (size_t i = 0; i < m_displayList.size(); i++)
	{
		// get the scale & translation of obj
		const XMVECTORF32 scale = { m_displayList[i].m_scale.x, m_displayList[i].m_scale.y, m_displayList[i].m_scale.z };
		const XMVECTORF32 location = { m_displayList[i].m_position.x, m_displayList[i].m_position.y, m_displayList[i].m_position.z };

		// convert euler rotaion into quaternmion for the rotation of obj - in rads
		XMVECTOR rotation = Quaternion::CreateFromYawPitchRoll(
			m_displayList[i].m_orientation.y * 3.1415 / 180,
			m_displayList[i].m_orientation.x * 3.1415 / 180,
			m_displayList[i].m_orientation.z * 3.1415 / 180);

		// create local transformation matrix
		XMMATRIX localTransform = m_world * XMMatrixTransformation(g_XMZero, Quaternion::Identity, scale, g_XMZero, rotation, location);

		// deproject ray vectors to worldspace
		XMVECTOR nearPoint = XMVector3Unproject(
			nearSource,
			0.f, 0.f,
			sceenDims.right, sceenDims.bottom,
		...
		// make vector between points - eg our ray to trace
		XMVECTOR selectorRay = farPoint - nearPoint;
		selectorRay = XMVector3Normalize(selectorRay);

		// loop through mesh list to find objects
		for (size_t j = 0; j < m_displayList[i].m_model.get()->meshes.size(); j++)
		{
			// responses?
			if (m_displayList[i].m_model.get()->meshes[j]->boundingBox.Intersects(nearPoint, selectorRay, selectedDist) && m_displayList[i].m_ID != -1)
				if (selectedDist < shortestDist) {
					selectedID = m_displayList[i].m_ID;
					shortestDist = selectedDist;
				}
		}
	}
...
bool Game::RayIntersectsTriangle(XMVECTOR RaySource, XMVECTOR RayVector, XMVECTOR Verticies[3], XMVECTOR& IntersectionPoint)
{
	...
	H = XMVector3Cross(RayVector, Edge2);
	...
	V = F * XMVectorGetX(XMVector3Dot(RayVector, Q));

	if (V < 0.f || U + V > 1.f)
		return false;

	// Can now calculate T to get the intersection point
	float T = F * XMVectorGetX(XMVector3Dot(Edge2, Q));
	// Does ray intersect?
	if (T > Epsilon)
	{
		IntersectionPoint = RaySource + RayVector * T;
		return true;	// pog - ray intersects
	}
	else
		return false;	// sadge - line intersects but not ray
}
...
void Game::BuildDisplayList(std::vector<SceneObject> * SceneGraph)
{
	...
	for (int i = 0; i < numObjects; i++)
	{
		...
		if (SceneGraph->at(i).ID == m_selectedObject) {
			...
			//apply new texture to models effect
			objectHighlight.m_model->UpdateEffects([&](IEffect* effect) //This uses a Lambda function,  if you dont understand it: Look it up.
				{
					//auto lights = dynamic_cast<BasicEffect*>(effect);
					//if (lights)
					//{
